...
mod_clustering_server <- function(id, acp_results_reactive) {
  moduleServer(id, function(input, output, session) {
    ...
    hcpc_results <- eventReactive(input$run_hcpc, {
      ...
      nb_dim_requested <- input$hcpc_n_dim
      nb_clust <- input$hcpc_n_clust
      
      # Get ACTUAL max dimensions computed in PCA
      # Safely access eigenvalues - might not exist if PCA failed
      max_dim_pca <- tryCatch(ncol(res.pca$eig), error = function(e) 0)
      
      # --- VALIDATION FOR nb_dim ---
      # 1. Must be at least 2
      validate(need(nb_dim_requested >= 2, "Au moins 2 dimensions sont requises."))
      # 2. Cannot exceed available PCA dimensions
      validate(need(nb_dim_requested <= max_dim_pca,
                    paste0("Le nombre de dimensions choisi (", nb_dim_requested,
                           ") dépasse le nombre calculé par l'ACP (", max_dim_pca,"). Veuillez réduire.")))
      # 3. Cluster number must be >= 0
      validate(need(nb_clust >= 0, "Le nombre de clusters doit être >= 0."))
      # --- End VALIDATION ---
      ...
      result <- tryCatch({ FactoMineR::HCPC(res.pca, nb.clust = ifelse(nb_clust == 0, -1, nb_clust), consol = TRUE, graph = FALSE, metric = "euclidean", method = "ward", nb.par = nb_dim_to_use ) }, # Use validated dim number
                         error = function(e) { showNotification(paste("Erreur HCPC:", e$message), type="error", duration=10); return(NULL) })
      req(result); return(result) # Require non-NULL result
    }, ignoreNULL = TRUE)
