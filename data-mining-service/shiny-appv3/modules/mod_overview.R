...
mod_overview_server <- function(id, data_reactive, auth_summary_reactive, service_summary_reactive) {
  moduleServer(id, function(input, output, session) {
    ...
    # --- Value Boxes ---
    output$vb_total_revenue <- renderValueBox({
      df <- df_reactive()
      req(df, df$total_amount)
      total_rev <- sum(df$total_amount, na.rm = TRUE)
      valueBox(
        value = scales::dollar(total_rev, prefix = "", suffix = " €", big.mark = " ", decimal.mark = ","),
        subtitle = "Total Revenue (Sample)",
        icon = icon("euro-sign"),
        color = "purple"
      )
    })

    output$vb_total_transactions <- renderValueBox({
      df <- df_reactive()
      req(df)
      n_trans <- nrow(df)
      valueBox(
        value = formatC(n_trans, format="d", big.mark=" "),
        subtitle = "Number of Transactions (Sample)",
        icon = icon("receipt"),
        color = "blue"
      )
    })

    output$vb_unique_clients <- renderValueBox({
      df <- df_reactive()
      req(df, df$client_id)
      n_clients <- n_distinct(df$client_id, na.rm = TRUE)
      valueBox(
        value = formatC(n_clients, format="d", big.mark=" "),
        subtitle = "Number of Unique Clients (Sample)",
        icon = icon("users"),
        color = "yellow"
      )
    })
    ...
    output$plot_pie_auth_methods <- renderPlot({
      ...
      ggplot(summary_data, aes(x="", y=prop, fill=authentication_method)) +
        ...
        labs(fill="Method", title=NULL) + # Remove default title, set legend title
    ...
    output$plot_bar_service_requests <- renderPlot({
      ...
      ggplot(summary_data, aes(x = reorder(service_name, count), y = count, fill=service_name)) +
        geom_col() +
        coord_flip() +
        labs(title = NULL, x = "Service", y = "Number of Requests") +
        theme_minimal() +
    ...
    output$plot_boxplot_age_montant <- renderPlot({
      ...
      ggplot(df_plot, aes(x = age_group, y = total_amount, fill = age_group)) +
        ...
        labs(title = NULL, x = "Age Group", y = "Transaction Amount (€)") +
        theme_minimal() + theme(legend.position = "none")
    })

    # --- Plot: Season/Amount Bar Chart (using renderPlot) ---
    output$plot_barchart_saison_montant <- renderPlot({
      df <- df_reactive()
      req(df, "saison" %in% colnames(df), "total_amount" %in% colnames(df))
      df_summary <- df %>%
        filter(!is.na(saison), saison != "Unknown", saison != "", !is.na(total_amount)) %>%
        group_by(saison) %>%
        summarise(montant_agg = sum(total_amount, na.rm = TRUE), .groups = 'drop') %>%
        filter(montant_agg > 0)
      ...
      ggplot(df_summary, aes(x = reorder(saison, -montant_agg), y = montant_agg, fill = saison)) + # Removed text aes
        geom_col(na.rm = TRUE) +
        scale_y_continuous(labels = scales::comma_format(big.mark = " ", decimal.mark = ",")) +
        labs(title = NULL, x = "Product Season", y = "Total Amount (€)") +
    ...
    output$plot_barchart_region_montant <- renderPlot({
      ...
      df_summary <- df %>%
        filter(!is.na(region), region != "Unknown", region != "", !is.na(total_amount)) %>%
        group_by(region) %>%
        summarise(montant_agg = sum(total_amount, na.rm = TRUE), .groups = 'drop') %>%
        filter(montant_agg > 0)
      req(nrow(df_summary) > 0) # Check summary has rows
      ...
      ggplot(df_summary, aes(x = reorder(region, -montant_agg), y = montant_agg, fill = region)) + # Removed text aes
        geom_col(na.rm = TRUE) +
        scale_y_continuous(labels = scales::comma_format(big.mark = " ", decimal.mark = ",")) +
        labs(title = NULL, x = "Client Region", y = "Total Amount (€)") +
        theme_minimal() + theme(legend.position = "none", axis.text.x = element_text(angle = 45, hjust = 1))
    })
    ...
    output$head_data <- DT::renderDataTable({
      ...
      DT::datatable(
        df,
        options = list(scrollX = TRUE, pageLength = 5, lengthMenu = c(5, 10, 25, 50)),
        rownames = FALSE,
        filter = 'top'
      )
    })
